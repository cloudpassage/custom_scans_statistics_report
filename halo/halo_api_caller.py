...
class HaloAPICaller(object):
    ...
    def get_server_scans(self, server_id, module_name):
        url = "%s:%d/%s/scans?server_id=%s&module=%s" % (
            self.halo_api_hostname, self.halo_api_port, self.halo_api_version, server_id, module_name)
        (data, auth_error) = self.do_get_request(url, self.halo_api_auth_token)
        if data:
            return json.loads(data), auth_error
        else:
    ...
    def get_server_scans_per_page(self, server_id, module_name, page):
        url = "%s:%d/%s/scans?server_id=%s&module=%s&per_page=100&page=%s" % (
            self.halo_api_hostname, self.halo_api_port, self.halo_api_version, server_id, module_name, page)
        (data, auth_error) = self.do_get_request(url, self.halo_api_auth_token)
        if data:
            return json.loads(data), auth_error
        else:
